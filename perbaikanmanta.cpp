...
void completeOrder() {
    ...
    // Indeks pesanan
    cout << "Masukkan nomor indeks pesanan untuk diselesaikan: ";
    int orderIndex;
    cin >> orderIndex;

    // Validasi indeks pesanan
    if (orderIndex < 0 || orderIndex >= matchingCount) {
        cout << "Nomor indeks tidak sesuai!\n";
        system("pause");
        return;
    }
        ...
        if (orders[selectedOrder]->provider == currentUser->username) {
            for (int j = 0; j < userCount; ++j) {
                if (users[j]->username == orders[selectedOrder]->username) {
                    users[j]->balance += orders[selectedOrder]->price;
                    cout << "Pesanan dibatalkan! Refund uang ke pemesan berhasil.\n";
                    system("pause");
                    return;
                }
            }
